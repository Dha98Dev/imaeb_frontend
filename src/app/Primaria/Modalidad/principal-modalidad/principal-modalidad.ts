...
export class PrincipalModalidad {
  constructor(
    ...
    private estadistica: GetEstadisticaService,
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      this.modalidad =
        this.cryptoJs.Desencriptar(this.cryptoJs.fromBase64Url(params.get('mod_des')!)) || '';
  ...
  getPromedioMateriasByModalidadAndNivel(): void {
    ...
    const requests$ = materias.map((m) =>
      this.estadistica.getPromedioEstatalByNivel({ materiaId: m, nivelId, modalidadId }).pipe(
        map((resp: any[]) => ({
            ...
            switchMap((zonas: any[]) => {
              ...
              const zonaRequests = zonas.map((z: any) =>
                this.catalogo
                  ...
                  .pipe(
                    map((resp3: any) => {
                      const ccts: CentrosTrabajo[] =
                        resp3?.cct ?? resp3?.centrosTrabajo ?? resp3?.data ?? [];
  ...
  cargarResultadosPromedios(): void {
    ...
    const promNivel$ = this.estadistica
      .getPromedioEstatalByNivel({ nivelId })
      .pipe(map((resp: any[]) => resp?.[0]?.promedio ?? 0));

    const promModalidad$ = this.estadistica
      .getPromedioEstatalByNivel({ nivelId, modalidadId })
      .pipe(map((resp: any[]) => resp?.[0]?.promedio ?? 0));
          ...
          const sectores$ = sectores.map((sectorItem) => {
            ...
            const zonas$ = sectorItem.zonas.map((z) => {
              ...
              const ccts$ = z.cct?.length
                ? forkJoin(
                    z.cct.map((ct) =>
                      this.estadistica.getPromedioEstatalByNivel({ escuelaId: ct.id }).pipe(
                        map((r: any[]) => ({
                          cct: ct.cct,
        ...
        next: (rows: resultadosModalidad[]) => {
          ...
          this.tableDinamica = {
            columns: [
              ...
              { key: 'sector', label: 'Sector', type: 'number', filterable: false },
              ...
              { key: 'zona', label: 'Zona', type: 'number', filterable: false },
              ...
              { key: 'cct', label: 'Centro de Trabajo', type: 'text', filterable: false },
