...
export class CatalogoService {
  ...
  getLocalidades(idMun: number): Observable<MunicipiosOrLocalidades[]> {
    let params = new HttpParams().set('municipioId', idMun);
    return this.http.get<MunicipiosOrLocalidades[]>(
      this.url + 'api/catalogos/localidades/municipio',
      { params },
    );
  }
  getCiclos(): Observable<CatalogoCiclos[]> {
    return this.http.get<CatalogoCiclos[]>(this.url + 'api/catalogos/ciclos');
  }
